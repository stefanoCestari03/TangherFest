...
export default function FormPage({ squadre, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Emails — errore silenzioso, non blocca l'iscrizione
      const ejsOpts = { publicKey: import.meta.env.VITE_EMAILJS_PUBLIC_KEY }
      const tipoLabel = nuova.tipo === 'tesserata' ? 'Pro' : 'Amatori'
      try {
        // Notifica all'organizzatore
        await emailjs.send(
          import.meta.env.VITE_EMAILJS_SERVICE_ID,
          import.meta.env.VITE_EMAILJS_TEMPLATE_ID,
          { nomeSquadra: nuova.nomeSquadra, tipo: tipoLabel },
          ejsOpts
        )
      } catch (_) {}
      try {
        // Conferma alla squadra
        await emailjs.send(
          import.meta.env.VITE_EMAILJS_SERVICE_ID,
          import.meta.env.VITE_EMAILJS_TEMPLATE_CONFIRM_ID,
          {
            to_email:    nuova.email,
            referente:   nuova.referente,
            nomeSquadra: nuova.nomeSquadra,
            tipo:        tipoLabel,
          },
          ejsOpts
        )
